...
class GameInfo:
    def __init__(self):
        self.TitleId: str = "5C19E"  # Playfab Title Id
        self.SecretKey: str = "HFDWG5RBE7QDPQ6HZADHP7ZKNP48N6JUX6DFKOGOHMB1TFS6NY"  # Playfab Secret Key
        self.OculusApiKey: str = ""  # Oculus/Graph API
        self.supported_platforms = ["Quest", "Pico", "Oculus", "Android"]
...
def detect_headset_type(platform: str, oculus_id: str = None, user_agent: str = None) -> str:
    """Определяет тип VR-гарнитуры"""
    platform_lower = platform.lower() if platform else ""
    
    if platform_lower in ["quest", "oculus"]:
        return "quest"
    elif platform_lower == "pico":
        return "pico"
    elif "pico" in (user_agent or "").lower():
        return "pico"
    elif oculus_id and ("pico" in oculus_id.lower() or oculus_id.startswith("PICO")):
        return "pico"
    else:
        return "unknown"

# Validate Oculus nonce (только для Quest)
def get_is_nonce_valid(nonce: str, platform: str, oculusId: str) -> bool:
    if not settings.OculusApiKey:
        logger.warning("Oculus API Key not set, skipping nonce validation")
        return True
    
    headset_type = detect_headset_type(platform, oculusId)
    
    
    if headset_type == "pico":
        logger.info(f"Skipping Oculus nonce validation for Pico device: {oculusId}")
        return True
    
    # Для Quest выполняем обычную проверку
    if headset_type == "quest":
        try:
            req = requests.post(
                url=f'https://graph.oculus.com/user_nonce_validate?nonce={nonce}&user_id={oculusId}&access_token={settings.OculusApiKey}',
            ...
            if req.status_code == 200:
                ...
                logger.error(f"Oculus nonce validation failed: {req.status_code}")
                return False
        except Exception as e:
            logger.error(f"Oculus nonce validation error: {e}")
            return False
    ...
    return """
        <html>
            ...
            <body style="font-family: 'Inter', sans-serif; background: linear-gradient(to bottom, #004d00, #00cc00); color: white; text-align: center; padding: 50px;">
                ...
                <p style="font-size: 14px; opacity: 0.8;">Supported platforms: Quest, Pico</p>
            </body>
        </html>
    """
...
def playfab_authentication():
    ...
    # Определяем необходимые поля в зависимости от платформы
    platform = rjson.get("Platform", "")
    headset_type = detect_headset_type(platform, rjson.get("OculusId"))
    
    if headset_type == "quest":
        required_fields = ["Nonce", "AppId", "Platform", "OculusId"]
    elif headset_type == "pico":
        # Для Pico Nonce и OculusId могут быть опциональными
        required_fields = ["AppId", "Platform"]
    else:
        required_fields = ["AppId", "Platform"]
    ...
    # Валидация nonce только для Quest
    if headset_type == "quest":
        if not get_is_nonce_valid(rjson.get("Nonce", ""), platform, rjson.get("OculusId", "")):
            return jsonify({"Message": "Invalid nonce", "Error": "BadRequest-InvalidNonce"}), 401
    
    # Генерация ServerCustomId в зависимости от платформы
    if headset_type == "quest":
        server_custom_id = "OCULUS" + rjson.get("OculusId", "")
    elif headset_type == "pico":
        # Для Pico используем PICO префикс или другое значение
        pico_id = rjson.get("OculusId") or rjson.get("PicoId") or rjson.get("UserId") or str(uuid.uuid4())
        server_custom_id = "PICO" + pico_id
    else:
        # Для других платформ
        user_id = rjson.get("OculusId") or rjson.get("UserId") or str(uuid.uuid4())
        server_custom_id = platform.upper() + user_id
...
def titledata():
    response_data = {
        ...
        "BundleBoardSign": "<color=#ff4141>DISCORD.GG/SYXNRWT9JJ</color>",
        "BundleKioskButton": "<color=#ff4141>DISCORD.GG/SYXNRWT9JJ</color>",
        "BundleKioskSign": "<color=#ff4141>DISCORD.GG/SYXNRWT9JJ</color>",
        "BundleLargeSign": "<color=#ff4141>DISCORD.GG/SYXNRWT9JJ</color>",
        "SeasonalStoreBoardSign": "<color=red>OUR DISCORD SERVER IS BELOW:</color>\n<color=blue>DISCORD.GG/SYXNRWT9JJ</color>",
        # Text and Messages
        "EmptyFlashbackText": "\nFLOOR TWO NOW OPEN\n       YAPPY",
        "MOTD": "<color=#FFC0CB>WELCOME TO PROJECT FUSION!</color>\n\n<color=#0099c2>CURRENT UPDATE: HOVERBOARD REVAMP</color>\n<color=#41ff80>T14 MADE THIS MOTD</color>\n<color=#6417ff>OUR DEVELOPERS ARE: KIRPICH4 & OZE & T14</color>\n<color=#ac1a00>CREDITS FOR GAMES OG OS: OZE</color>\n\n<color=#91A3B0>OUR DISCORD: DISCORD.GG/SYXNRWT9JJ</color>\n<color=#00FF00>OWNER: KIRPICH4, OZE</color>\n<color=#8615F7>DEVELOPER: T14</color>\n<color=#DC160F>ADMIN: TAPO4EK, SNEZHOK, GOSHANCHIK</color>\n<color=#1773FD>MOD: PEPPINO, NAPITOK, POCUPATEL, MOLODISHKA, KFC</color>",
        "TOBAlreadyOwnCompTxt": "DISCORD.GG/SYXNRWT9JJ",
        "TOBAlreadyOwnPurchaseBundle": "PROJECT FUSION",
        "TOBDefCompTxt": "DISCORD.GG/SYXNRWT9JJ",
        "TOBDefPurchaseBtnDefTxt": "PROJECT FUSION",
        # Legal and Versions
        "EnableCustomAuthentication": True,
        "LatestPrivacyPolicyVersion": "2024.09.20",
        "LatestTOSVersion": "2024.09.20",
        "TOS_2024.09.20": "DISCORD.GG/SYXNRWT9JJ",
        ...
        "PingThreshold": 200,
        "RegionPriority": ["US", "EU", "AS", "RU"],
        "EnableSpectatorMode": True,
        "TutorialEnabled": True,
        "NewsFeedUrl": "https://discord.gg/syxNRWT9jj",
        ...
        # Поддержка платформ
        "SupportedPlatforms": ["Quest", "Pico"],
        "PlatformSpecificSettings": {
            "Quest": {"controllerType": "Touch", "handTracking": True},
            "Pico": {"controllerType": "Pico4", "handTracking": False}
        },
        # New Configurations
        "EnableSeasonalEvents": True,
        "SeasonalEventName": "HOVERBOARD REVAMP",
        ...
        "BanAppealUrl": "https://discord.gg/syxNRWT9jj",
        ...
        "ServerRegionLatencyCaps": {"US": 150, "EU": 200, "AS": 250, "RU": 200},
        "EnableVoiceModeration": True,
        "VoiceModerationThreshold": 0.9,
        "EnablePlayerFeedback": True,
        "FeedbackSubmissionUrl": "https://discord.gg/MR7GZcWP4a"
    }
...
def consume_oculus_iap():
    rjson = request.get_json()
    required_fields = ["userToken", "userID", "nonce", "sku", "platform"]
    missing_fields = validate_input(rjson, required_fields)
    if missing_fields:
        return jsonify({"error": f"Missing required fields: {', '.join(missing_fields)}"}), 400

    platform = rjson.get("platform")
    headset_type = detect_headset_type(platform)
    
    if headset_type == "pico":
        logger.info(f"Pico IAP consumption requested: {rjson}")
        return jsonify({
            "result": True,
            "message": "Pico IAP processed (placeholder)",
            "platform": "Pico"
        })
    ...
    response = requests.post(
        url=f"https://graph.oculus.com/consume_entitlement?nonce={nonce}&user_id={user_id}&sku={sku}&access_token={settings.OculusApiKey}",
        headers={"content-type": "application/json"},
    )
...
def photonauth():
    ...
    headset_type = detect_headset_type(platform, user_id)
    
    if platform not in ["Quest", "Pico"]:
        return jsonify({'Error': 'Bad request', 'Message': f'Unsupported platform: {platform}'}), 403

    if headset_type == "quest" and nonce:
        if not get_is_nonce_valid(nonce, platform, user_id):
            return jsonify({'Error': 'Bad request', 'Message': 'Invalid nonce!'}), 304
    elif headset_type == "pico":
        logger.info(f"Pico device authentication: {user_id}")
    ...
    if req.status_code == 200:
        ...
        logger.info(f"Authenticated user {user_id.lower()} with nickname: {nick_name}, platform: {platform}")
        
        return_function_json("SigmaAuth", {"platform": platform, "headsetType": headset_type}, user_id)
...
@app.route("/api/GetPlatformInfo", methods=["GET"])
...
@app.route("/api/GetHeadsetConfig", methods=["POST"])
def get_headset_config():
    rjson = request.get_json()
    platform = rjson.get("platform", "Quest")
    headset_type = detect_headset_type(platform)
    ...
    return jsonify({
        "platform": platform,
        "headsetType": headset_type,
        "config": config.get(headset_type, config["quest"])
    })
...
if __name__ == "__main__":
    ...
    logger.info(f"Supported platforms: {settings.supported_platforms}")
